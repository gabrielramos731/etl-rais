...
conn = psycopg2.connect(
    host="localhost",
    database="postgres",  # banco já criado no DBeaver
    user="postgres",
    password="230204",
    port="5433"
)
...
df = pd.read_csv('C:/dev/ndti/rais/indices2/secao/fact_sec_municipio.csv', sep=';')
dados = list(df.itertuples(index=False, name=None))
cur.executemany(
...
df = pd.read_csv('C:/dev/ndti/rais/indices2/divisao/fact_div_municipio.csv', sep=';')
dados = list(df.itertuples(index=False, name=None))
cur.executemany(
